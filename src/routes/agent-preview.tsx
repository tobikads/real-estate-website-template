...
type DemoMode = "idle" | "missed" | "zillow";
...
const zillowLead = {
  ...
  time: "Just now",
  textToLead: `Hi Marcus, this is ${realtorFirstName}. I saw your Zillow request about 1020 Ivy Ridge Court. I can help with availability, showings, or similar homes. This quick link helps me send the right next step: [Open property-aware buyer flow]. Reply STOP to opt out.`,
};
      ...
      <main className="pt-28 lg:pt-32">
            ...
            <div className="mt-6 grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
              <div>
                ...
                <p className="mt-6 max-w-2xl text-base font-light leading-relaxed text-stone-600">
                  This shows the realtor what happens after a missed call or a Zillow inquiry —
                  how each becomes a cleaner, property-aware lead.
                </p>
              </div>
              <div className="border border-stone-200 bg-white p-5 shadow-sm">
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                  <button
                    type="button"
                    onClick={() => setMode((m) => (m === "missed" ? "idle" : "missed"))}
                    className={`inline-flex min-h-12 items-center gap-3 px-5 py-3 text-[11px] uppercase tracking-[0.26em] transition-colors ${
                      mode === "missed"
                        ? "bg-stone-700 text-[#faf7f2] hover:bg-stone-600"
                        : "bg-stone-900 text-[#faf7f2] hover:bg-stone-800"
                    }`}
                  >
                    <PhoneMissed className="h-4 w-4" strokeWidth={1.5} />
                    {mode === "missed" ? "Missed Call Active" : "Simulate Missed Call"}
                  </button>
                  <button
                    type="button"
                    onClick={() => setMode((m) => (m === "zillow" ? "idle" : "zillow"))}
                    className={`inline-flex min-h-12 items-center gap-3 px-5 py-3 text-[11px] uppercase tracking-[0.26em] transition-colors ${
                      mode === "zillow"
                        ? "bg-stone-700 text-[#faf7f2] hover:bg-stone-600"
                        : "border border-stone-900 bg-transparent text-stone-900 hover:bg-stone-900 hover:text-[#faf7f2]"
                    }`}
                  >
                    <Home className="h-4 w-4" strokeWidth={1.5} />
                    {mode === "zillow" ? "Zillow Lead Active" : "Simulate Zillow Inquiry"}
                  </button>
                  {mode !== "idle" && (
                    <button
                      type="button"
                      onClick={() => setMode("idle")}
                      className="inline-flex min-h-12 items-center gap-2 px-3 py-3 text-[11px] uppercase tracking-[0.26em] text-stone-600 hover:text-stone-900"
                    >
                ...
                <p className="mt-4 text-xs font-light leading-relaxed text-stone-500">
                  Demo only. In a live version, these events would come from a phone provider or
        ...
        <section className="px-6 py-12 lg:px-10 lg:py-16">
          <div className="mx-auto grid max-w-6xl gap-6 lg:grid-cols-[1fr_0.9fr]">
            {mode === "zillow" ? <ZillowLeadPanel /> : <MissedCallPanel active={mode === "missed"} />}
            <AlertPreview mode={mode} />
          </div>
        </section>
...
function MissedCallPanel({ active }: { active: boolean }) {
  return (
    <article className="border border-stone-200 bg-white p-6 shadow-sm sm:p-8">
      <div className="flex items-start justify-between gap-5">
        <div>
          ...
          <h2 className="mt-4 font-serif text-3xl text-stone-950">
            {active ? missedCallLead.phone : "Ready to capture a missed call"}
          </h2>
        </div>
        <div
      ...
      <dl className="mt-8 grid gap-4 sm:grid-cols-2">
        <PreviewField label="Source" value={active ? missedCallLead.source : "Waiting"} />
        <PreviewField label="Time" value={active ? missedCallLead.time : "Not triggered"} />
        <PreviewField
          label="Status"
          value={active ? missedCallLead.status : "No missed call yet"}
        />
        <PreviewField
          label="Suggested action"
          value={active ? "No manual step yet" : "Simulate the call first"}
        />
      </dl>
...
function ZillowLeadPanel() {
  return (
    <article className="border border-stone-200 bg-white p-6 shadow-sm sm:p-8">
      <div className="flex items-start justify-between gap-5">
        <div>
          ...
          <p className="mt-2 text-sm font-light text-stone-600">
            asked about{" "}
            <span className="text-stone-900">{zillowLead.property}</span>
          </p>
        </div>
      ...
      <div className="mt-7 border-l-2 border-stone-900 pl-4 text-sm font-light italic leading-relaxed text-stone-700">
        "{zillowLead.message}"
      </div>
      ...
      <div className="mt-5 border border-stone-200 p-5">
        ...
        <p className="mt-3 text-sm font-light leading-relaxed text-stone-700">
          The buyer flow opens with the Zillow property already attached, so Marcus doesn't start
          from zero. A few quick answers tell {realtorFirstName} whether to confirm a showing or
          send close alternatives.
        </p>
        <Link
          to="/buyer"
          search={{ ref: "zillow" }}
          className="mt-5 inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.28em] text-[#faf7f2] bg-stone-900 px-5 py-3 hover:bg-stone-800 transition-colors"
        >
...
function AlertPreview({ mode }: { mode: DemoMode }) {
  const isZillow = mode === "zillow";
  const isMissed = mode === "missed";

  let subject = "Subject: Missed lead call from your website";
  let body =
    "When a call is missed, this panel shows the private alert the agent would receive.";

  if (isMissed) {
    body = `You missed a call from ${missedCallLead.phone}. A Start Here text was automatically sent so they can choose Buyer, Seller, or Question. If they fill out a form, you'll get their info and the suggested next step.`;
  } else if (isZillow) {
    subject = "Subject: New Zillow inquiry — 1020 Ivy Ridge Court";
    body = `${zillowLead.name} asked about 1020 Ivy Ridge Court through Zillow. A property-aware text was automatically sent with a link to a buyer flow that already has the home attached. Once Marcus answers a few questions, you'll see availability fit or close alternatives.`;
  }
    ...
    <article className="border border-stone-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      <div className="mt-6 rounded-lg border border-stone-200 bg-stone-50 p-5 text-sm font-light leading-relaxed text-stone-700 shadow-sm">
        <p className="text-[10px] uppercase tracking-[0.25em] text-stone-400">
          Email - {mode === "idle" ? "preview" : "just now"}
        </p>
...
function TimelinePanel({ mode }: { mode: DemoMode }) {
  const active = mode !== "idle";
  const isZillow = mode === "zillow";

  const items = isZillow
    ? [
        {
          icon: Home,
          title: "Zillow inquiry",
          body: "Marcus asked about 1020 Ivy Ridge Court through Zillow.",
          active: true,
        },
        {
          icon: Bell,
          title: "Agent alert",
          body: "Agent is notified privately with the property and lead context.",
          active: true,
        },
        {
          icon: MessageSquareText,
          title: "Property-aware text",
          body: "Marcus receives a text that already references the home he asked about.",
          active: true,
        },
        {
          icon: Check,
          title: "Lead enriched",
          body: "Buyer flow attaches the property and gathers a few key answers.",
          active: false,
        },
      ]
    : [
        {
          icon: PhoneMissed,
          title: "Missed call",
          body: active ? "Caller reached out but did not get an answer." : "Waiting for call.",
          active,
        },
        {
          icon: Bell,
