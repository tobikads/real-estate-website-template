...
const topo = JSON.parse(fs.readFileSync('/tmp/us10m.json','utf8'));
...
for (const [name,fips] of Object.entries(want)) {
  const f = allCounties.features.find(x => x.id === fips || x.id === Number(fips));
...
for (const [name,f] of Object.entries(feats)) {
  out[name] = { d: path(f), cx: path.centroid(f)[0], cy: path.centroid(f)[1] };
}
